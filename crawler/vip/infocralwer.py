...
cur_detailed_data=[]
def crawl_detail(search_keyword):
    ...
    def select(response):
        if response.url.find('v5?') !=-1:
            global cur_detailed_data
            json_data=json.loads(response.body().decode())['data']
...
#            print(json_data)            
    ...
    def crawl(playwright):
        ...
        for index_data in tqdm(index_datas):
            ...
            _i+=1
            if(_i==len(index_datas) or _i==len(index_datas)-1 or _i%60==0):
                with open(f'detail-{search_keyword}.json', 'w', encoding='utf-8') as f:
